...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/18 22:29:28 by yoel-idr          #+#    #+#             */
/*   Updated: 2023/03/19 15:18:54 by yoel-idr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "leet3d.h"

static void	setting_sounds(t_sound *sound)
{
	if (sound->up)
...
void	*sound_(void *ptr)
{
	...
	while (true)
	{
		...
		if (sound->over && sound->sfx)
			(system(S_OVER), sound->over = 0);
		...
		if (sound->sfx && sound->damage)
			(system(S_DAMAGE), sound->damage = 0);
	}
	return (NULL);
}
