...
/*   Updated: 2023/03/10 16:03:55 by yoel-idr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	_offset(t_leet *leet, int i, int top, char v)
{
	...
	if (v == 'X')
	{
		if (leet->ray.player.rays[i].hit_side == HORIZONTAL_HIT)
			offset = (int)leet->ray.player.rays[i].x_wall_hit % TILE_SIZE;
		else
			offset = (int)leet->ray.player.rays[i].y_wall_hit % TILE_SIZE;
	}
	else
	{
		offset = (top + (leet->ray.player.rays[i].wall_height / 2)
				- (WINDOW_HEIGHT / 2)) * ((double)TEXTURE_DIMENTION
				/ leet->ray.player.rays[i].wall_height);
	}
	return (offset);
}
